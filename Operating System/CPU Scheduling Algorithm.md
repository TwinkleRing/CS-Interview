 ...
 * 선점형 : 현재 CPU에서 실행중인 프로세스의 남은 CPU burst time보다 더 짧은 CPU burst의 프로세스가 도착하면 CPU를 빼앗게 됩니다.<br>
              일반적인 시분할 환경에서는 계속 새로운 프로세스가 도착하므로 선점형 방식이 평균 대기시간을 가장 많이 줄일 수 있는 방식이 됩니다.<br>
...
#### 특징 
 * 가장 현대적인 CPU 스케줄링으로 **시분할 시스템**을 위해 설계되었다.
...
#### 주의할 점
 * 할당 시간(time quantum)이 너무 작으면 계속되는 Context Switch로 오버헤드가 발생합니다.
 * 할당 시간(time quantum)이 너무 커지면 FCFS와 같아져 비효율적이게 됩니다.
 * 그러므로 적절한 할당 시간(time quantum)을 설정하는 것이 중요합니다.
...
#### 특징 
 * 어떤 프로세스인지에 따라 여러 종류의 그룹으로 나누고 여러 개의 큐에 다양한 스케줄링 알고리즘을 적용하는 기법입니다.
 * 성격이 다른 프로세스들을 별도로 관리하고 프로세스의 성격에 맞는 스케줄링을 적용하기 위해 준비 큐를 여러 개 사용합니다.
 * 사용자와 상호작용하는 앞단의 대화형 프로세스는 중요하기에 우선순위가 높아 라운드 로빈 스케줄링을 사용합니다.
 * 백그라운드에서 돌아가는 일괄처리형(Batch) 프로세스는 우선순위가 낮아 가장 간단한 FCFS 방식으로 처리합니다.
...
#### 특징
 * 다단계 큐의 형평성 문제를 완화한 스케줄링 기법입니다.
 * 다단계 피드백 큐 알고리즘에서는 우선 순위의 변동이 가능하여 큐 사이의 이동이 가능합니다.
 * 다단계 큐에 에이징 기법을 더한 방법으로, 우선순위가 낮은 큐에서 오랫동안 기다렸으면, 우선순위가 높은 큐로 승격시키는 방식이다.
