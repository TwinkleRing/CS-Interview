...
---
1. 레지스터와 캐시 =  레지스터와 캐시는 CPU 내부에 존재하기 때문에 CPU가 아주 빠르게 접근 할 수 있다. <br>
2. 메모리 = 메모리는 CPU 외부에 존재한다. 그래서 레지스터와 캐시보다 더 느리게 접근 할 수 밖에 없다. </br>
3. 하드 디스크 = 하드 디스크에는 CPU가 직접 접근할 수 없다. 그래서 CPU가 하드 디스크에 접근하기 위해서는 하드 디스크의 데이터를 메모리로 먼저 이동시키고나서 메모리에 접근해야 한다.
